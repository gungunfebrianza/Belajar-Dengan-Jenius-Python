...
class TranslationFile(User):
    ...
    def _make_message_el(self, message):
        ...
        if message.embedded_comments.extra_comments:
            ...
            extracomment_el.text = cls._get_message_extra_comments(message)
    ...
    def _summary(self):
        ...
        def summary(line):
            ...
            if not heading:
                print("Summary of changes to {ts}:".format(ts=self._ts_file))
                heading = True
        ...
        if not heading:
            print("{ts} was unchanged".format(ts=self._ts_file))
